...
class GARCHModel:
    ...
    def fit(self, returns, verbose=False):
        ...
        self.model = arch_model(
            returns_scaled, vol="Garch", p=self.p, q=self.q, dist="normal"
        )
...
class EGARCHModel:
    ...
    def fit(self, returns, verbose=False):
        ...
        self.model = arch_model(
            returns_scaled, vol="EGARCH", p=self.p, o=self.o, q=self.q, dist="normal"
        )
...
class HARRVModel:
    ...
    def forecast(self, rv_history):
        ...
        features = np.array(
            [1.0, rv_history[-1], np.mean(rv_history[-5:]), np.mean(rv_history[-22:])]
        )
